...
除了上传 ZIP 文件，你也可以扫描服务器上已有的 MIB 目录：

1. 点击"品牌包管理"标签页
2. 在"服务器目录扫描"区域，点击"扫描目录"按钮
3. 系统会扫描配置的 MIB 路径（默认 `/etc/snmp/mibs`）
4. 找到的 ZIP 文件会显示在列表中
5. 点击"解压"按钮即可导入 MIB 文件

**文件夹浏览功能：**

系统支持完整的文件夹浏览功能，可以方便地管理按品牌分类的 MIB 文件：

1. **配置根路径**：
   - 在路径输入框中输入 MIB 文件根目录，例如 `/opt/mibs`
   - 点击"扫描"按钮

2. **浏览文件夹**：
   - 扫描结果会显示所有子文件夹和 ZIP 文件
   - 点击文件夹图标（蓝色）可以进入子目录
   - 点击"← 返回上一级"可以返回上级目录
...
**工作原理：**
- Docker 容器已映射宿主机根目录为 `/host`（只读）
- 后端自动将路径转换为容器内路径（如 `/opt/mibs` → `/host/opt/mibs`）
- 支持扫描宿主机的任意目录
- 解压的 MIB 文件存储在容器的 `/app/data/mibs` 目录（可读写）

**注意事项：**
- 确保宿主机上的目录存在且有读取权限
- 路径必须是绝对路径（以 `/` 开头）
- 扫描是只读操作，不会修改宿主机文件
- 解压的 MIB 文件会存储在应用的 data 目录中
